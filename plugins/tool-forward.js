const { cmd } = require("../command"); const { downloadContentFromMessage } = require("@whiskeysockets/baileys");

const SAFETY = { MAX_JIDS: 20, BASE_DELAY: 2000, EXTRA_DELAY: 4000, };

cmd({ pattern: "forward", alias: ["fwd"], desc: "Forward quoted media (including big documents/videos) to multiple groups safely", category: "owner", filename: __filename, }, async (client, message, match, { isOwner }) => { try { if (!isOwner) return message.reply("📛 Owner Only Command"); if (!message.quoted) return message.reply("📌 Reply to a media/document message");
...
} catch (error) { console.error("⚠️ Forward Error:", error); await message.reply( ❌ Error: ${error.message.substring(0, 100)}\nCheck:\n• Group access\n• File validity\n• Bot status ); } });
